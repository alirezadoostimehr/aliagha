...
func (zarinpal *Zarinpal) NewPaymentRequest(amount int, callbackURL string, description string, email string, mobile string) (paymentURL, authority string, err error) {
	if amount < 1 {
	...
	if err != nil {
		return "", "", errors.New("something went wrong in payment request")
	}
	if resp.Status == 100 {
		authority = resp.Authority
		paymentURL = zarinpal.PaymentEndpoint + resp.Authority
		return paymentURL, authority, nil
	} else {
